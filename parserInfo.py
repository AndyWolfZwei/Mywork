...
class Parser:
    def __init__(self):
        self.pattern = '包括|的(研究|教学)(工作)?|研究方向为?|从事|[\(\)（）\[\]\-、.：:;；,，。包括是长期主要【】]'
    ...
    def method1(self, words):
        print('在本句找方向中')
        print(words)
        dirs = re.findall('研究(?:方向|领域|兴趣)[为是]?[：:]?(.*?)[。.、]|研究(?:方向|领域|兴趣)[为是]?[：:]?(.*?)$|(?:主要|长期)?从事(.*?)的?研究', words)
        full_dir = []
        [full_dir.append(i) for i in dirs[0] if i]
        # print(full_dir)
        if len(full_dir[0]) > 3:
            u = re.split("[、， ；,;.。:：]", full_dir[0])  # 将第一句话分割 如果可以分割成多份 就判断字长度
            print('method1 话已分割')
            # print(u)
    ...
    def method2(self, list_info, n):
        ...
        try:
            ...
            temp1 = re.split('[,、，]',list_info[n + count])
            # print(temp1)
            if len(temp1) > 1:
                if len(temp1[0]) < 4:
                    return re.split(' ', re.sub(self.pattern, ' ', list_info[n + count]).strip())[1]
                else:
                    u = re.split(' ', re.sub(self.pattern, ' ', list_info[n + count]).strip())
                    if '方向' in u[0] and len(u[0])<5:
                        return u[1]
                    return u[0]
    ...
    def _search_dir(self, list_info):
        for n, words in enumerate(list_info):
            if re.findall("研究方向|研究领域|研究兴趣|研究专长|专业方向|(主要|长期)?从事|主讲课程", words):  # *****在list中再找关键字 ******   (match search findall)
                print('找到研究方向')
    ...
    @staticmethod
    def parser_qual(re_infos, l,name):
        ...
        templates = ['名誉教授', '研究教授', '访问教授', '杰出教授', '校聘教授', '正高研究员', '高级工程师', '讲席教授', '助理教授', '兼职教授', '副高研究员', '资深研究员',
                     '中级研究员', '副研究员', '研究员', '助理研究员', '助理工程师', '工程师', '教授','副教授', '讲师']
        uu = re.sub('[~，]', '', str(u))
        for n0, words0 in enumerate(l):
            if re.search('职 {0,5}称', words0):
                ...
                for template in templates:
                    if uu.count(template) != 0:
                        return template
                # except Exception as e:
                #     print('out of range in search prof,',e)
                #     continue
        else:
            return "教授"
